...
public class ScrollSnapRect : MonoBehaviour, IBeginDragHandler, IEndDragHandler, IDragHandler
{
    ...
    //------------------------------------------------------------------------
    [Header("GamePlay")]
    [SerializeField] GameObject[] btnRigh;
    ...
    void Start()
    {
     ...
     //   LerpToPage(PopupChapter.Instance.ShowPagePosition(_pageCount)); // Show Page Positon 

        // prev and next buttons
        if (nextButton)
            nextButton.GetComponent<Button>().onClick.AddListener(() => { NextScreen(true); });

        if (prevButton)
            prevButton.GetComponent<Button>().onClick.AddListener(() => { PreviousScreen(true); });
    }

    void InitPagePositon()
    {
       // PopupChapter.Instance.ShowPagePosition(_currentPage);
    }
    ...
    void Update()
    {
        ...
        if (_lerp)
        {
            ...
            if (_showPageSelection)
            {
             ...
             //   PopupChapter.Instance.GetChapterIndex(GetNearestPage());      //get chapterIndex
            }
    ...
    {
        foreach (var i in btnLeft)
        {
            foreach(var j in btnRigh)
            {
                j.SetActive(true);
                if (_currentPage == 0)
                {
    ...
    #region important
    private void NextScreen(bool isButton)
    {
        LerpToPage(_currentPage + 1);
        if (!isButton) return;
     //   HomeManager.Instance.OnClick();
    }

    //------------------------------------------------------------------------
    private void PreviousScreen(bool isButton)
    {
        LerpToPage(_currentPage - 1);
        if (!isButton) return;
       // HomeManager.Instance.OnClick();
    ...
    private int GetNearestPage()
    {
      ...
      //  PopupChapter.Instance.GetChapterIndex(nearestPage);      //get chapterIndex
